...
private fun handleScheduleReminder(
    ...
    coroutineScope.launch {
        try {
            if (isAutoGeneration) {
                // TODO()
                //viewModel.createAutoGeneratedPlan(deckId, source, startDate, endDate, preferredTime)
                //showToast(context, "План тренировок успешно составлен!")
                showToast(context, "Фичу пока не завезли")
            } else {
                val dateTimeInMillis = combineDateAndTime(selectedDate, selectedTime)
                if (dateTimeInMillis == null) {
                    showToast(context, context.getString(R.string.select_date_and_time))
                    return@launch
                }
                ...
                if (reminder == null) {
                    showToast(context,
                        context.getString(R.string.reminder_with_that_time_has_already_been_set))
                    return@launch
                }
                val reminderId = viewModel.insertReminder(reminder)
                viewModel.scheduleReminder(reminderId, dateTimeInMillis)
                showToast(context,
                    context.getString(R.string.training_has_been_successfully_scheduled))
            }
        } catch (e: Exception) {
